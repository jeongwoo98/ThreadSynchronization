...
static sem_t mutex, rw_mutex, fifoQueue_mutex;
...
//fifoQueue_mutex: to solve the starvation problem, we added another semaphore to 
//ensure fairness between the writer and reader threads. An additional queue semapahore
//that is called in beginning of the read and write operations of the threads, and released 
//right after each. 
...
int main(int argc, char *argv[])
{
  ...
  if (sem_init(&mutex, 0, 1) == -1 || sem_init(&rw_mutex, 0, 1) == -1 || sem_init(&fifoQueue_mutex,0,1)==-1)	//initialize semaphores
